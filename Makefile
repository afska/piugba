...
ifeq ($(ENV), debug)
	CXXFLAGS += -DIGNORE_LOCKS=true -DENABLE_STAGE_BREAK=false -DENABLE_BACKGROUND=false
else ifeq ($(ENV), development)
	CXXFLAGS += -DIGNORE_LOCKS=true -DENABLE_STAGE_BREAK=false -DENABLE_BACKGROUND=true
else
	CXXFLAGS += -DIGNORE_LOCKS=false -DENABLE_STAGE_BREAK=true -DENABLE_BACKGROUND=true
endif

...
inline bool PS2_DOWNLEFT() {
  return ps2Keyboard->arrows[0] || ps2Keyboard->arrows[5];
}

inline bool PS2_UPLEFT() {
  return ps2Keyboard->arrows[1] || ps2Keyboard->arrows[6];
}

inline bool PS2_CENTER() {
  return ps2Keyboard->arrows[2] || ps2Keyboard->arrows[7];
}

inline bool PS2_UPRIGHT() {
  return ps2Keyboard->arrows[3] || ps2Keyboard->arrows[8];
}

inline bool PS2_DOWNRIGHT() {
  return ps2Keyboard->arrows[4] || ps2Keyboard->arrows[9];
}
...
inline bool KEY_DOWNLEFT(u16 keys) {
  return ((keys & KEY_DOWN) | (keys & KEY_LEFT)) || PS2_DOWNLEFT();
}

inline bool KEY_UPLEFT(u16 keys) {
  return ((keys & KEY_L) | (keys & KEY_UP)) || PS2_UPLEFT();
}

inline bool KEY_CENTER(u16 keys) {
  return ((keys & KEY_B) | (keys & KEY_RIGHT)) || PS2_CENTER();
}

inline bool KEY_UPRIGHT(u16 keys) {
  return (keys & KEY_R) || PS2_UPRIGHT();
}

inline bool KEY_DOWNRIGHT(u16 keys) {
  return (keys & KEY_A) || PS2_DOWNRIGHT();
}

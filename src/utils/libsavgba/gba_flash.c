...
IWRAM_CODE
static void flash_memcpy(volatile unsigned char* dst,
...
IWRAM_CODE
static unsigned int flash_absmemcmp(const volatile unsigned char* dst,
                                    const volatile unsigned char* src,
                                    size_t size) {
  while (size-- > 0) {
    unsigned int a = *dst++;
...
static int wait_until(u32 addr, const u8* data, int timeout) {
  for (vu32 i = 0; i < LOOP_CNT_PER_MILLI_SECOND * timeout &&
                   flash_absmemcmp(&flash_mem[addr], data, 1);
       i++)
    ;

  if (flash_absmemcmp(&flash_mem[addr], data, 1)) {
    // Terminate Command after Timeout (only Macronix devices, ID=1CC2h)
    if (gFlashInfo.manufacturer == FLASH_MFR_MACRONIX &&
        gFlashInfo.device == FLASH_DEV_MX29L512)
...
int flash_init(u8 size) {
  ...
  // After detection of certain device types smaller wait values may be used for
  ...
  // one minor thing the atmel docs say: you have to wait 20ms when entering or
  // exiting ID mode.
  wait(20);
  ...
  // 128K sanyo flash needs to have the "exit ID mode" written TWICE to work. If
  // you only write it once, it will not exit ID mode. 64K sanyo flash has the
  // same device/manufacturer ID as the SST part.
  ...
  for (int i = 0; i < FLASH_CHIP_NUM; i++) {
    if (gFlashInfo.manufacturer == flash_chips[i].manufacturer &&
        gFlashInfo.device == flash_chips[i].device) {
...
int flash_reset() {
  ...
  // vba/vba-m fills erased memory with zeros.
  // https://github.com/visualboyadvance-m/visualboyadvance-m/pull/855
  if (err)
...
int flash_erase_and_write_atmel(u32 addr, u8* data) {
  ...
  // write 128 bytes
  for (int i = 0;
       i < FLASH_SECTOR_SIZE_128B - (addr & (FLASH_SECTOR_SIZE_128B - 1)); i++)
  ...
  return wait_until(addr | (FLASH_SECTOR_SIZE_128B - 1),
                    &data[(FLASH_SECTOR_SIZE_128B - 1) -
                          (addr & (FLASH_SECTOR_SIZE_128B - 1))],
                    20);
}
...
int flash_write_atmel(u32 addr, u8* data, size_t size) {
  ...
  for (int i = 0; i < sectors; i++) {
    err = flash_erase_and_write_atmel(addr + i * FLASH_SECTOR_SIZE_128B,
                                      &data[i * FLASH_SECTOR_SIZE_128B]);
    if (err)
      return err;
  }
